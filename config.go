...
// Config instantiation must be done in multiple steps
//  // try read config from file
//  err := ca.ReadConfigFromFile(*cfgPathPtr)
//  // whatever happens apply environment variables to config struct if any is present
// 	ca.ApplyEnv()
...
// ApplyEnv read environment variables and replace respective fields if present
...
func (ca *Cagent) CreateDefaultConfigFile(configFilePath string) error {
	var err error
	...
	err = toml.NewEncoder(f).Encode(&MinValuableConfig{
		HubURL:      ca.HubURL,
		HubUser:     ca.HubUser,
